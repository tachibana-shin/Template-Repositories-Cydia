...
function parseControl(control: string): any {
  const obj = {} as any;

  control.split("\n").filter(item => !!item.replace(/\s/g, "")).forEach(line => {
...
function stringifyControl(obj: any): string {
...
async function main() {
  if (fs.existsSync(PATH_PACKAGES) === false || fs.lstatSync(PATH_PACKAGES).isDirectory() === false) {
    throw new Error(`${join(__dirname, "packages")} is not directory`)
  }

  // auto fix packages
  const controlPackages = autoFixPermissionAndControlPackage(await getListPackages())
...
function createDepictionPackages(controls: ControlJSONFile[]): void {
  ...
  packages.forEach(versions => {
    ...
    fs.writeFileSync(join(__dirname, "controls", versions[0].control.Package), JSON.stringify(versions.map(item => {
...
function uniqueListPackages(controls: ControlJSONFile[]): Map < string, ControlJSONFile[] > {
  ...
  controls.forEach((controlFile) => {
    ...
    const indexInsert = packages.get(controlFile.control.Package) !.findIndex(({ control: { Version } }) => semver.gt(Version, controlFile.control.Version)) || packages.get(controlFile.control.Package) !.length

    packages.get(controlFile.control.Package) !.splice(indexInsert, 0, controlFile)
  })
...
async function getListPackages(): Promise<ControlJSONFile[]> {
  return fs.readdirSync(PATH_PACKAGES).map(filename => {
    ...
    return {
      ...
      control: parseControl(child_process.execSync(`dpkg -f "${pathToPackage}"`).toString())
    }
...
function autoFixPermissionAndControlPackage(controlPackages: ControlJSONFile[]): ControlJSONFile[] {
  return controlPackages.map((controlFile) => {
    ...
    control.Depiction = `${HOMEPAGE}/package/${control.Package}` // no report old verions package
...
function autoFixFilenamePackage(controlPackages: ControlJSONFile[]): void {
  controlPackages.forEach(controlFile => {
    if (basename(controlFile.filepath) !== `${controlFile.control.Package}@${controlFile.control.Version}.deb`) {
      // rename
      fs.renameSync(join(dirname(controlFile.filepath), `${controlFile.control.Package}@${controlFile.control.Version}.deb`), controlFile.filepath)
    }
  })
}
...
function updatePackage(filepath: string, control: ControlJSON): void {
  ...
  child_process.execSync(`dpkg-deb -R "${filepath}" "${PATH_TMP_UNPACK_DEBIAN}"`);
...
function createFileRelease(): void {
  fs.writeFileSync("Release", `Origin: Tachibana Shin (たちばなしん)
...
MD5Sum:
 ${md5file.sync(PATH_FILE_PACKAGES)} ${fs.statSync(PATH_FILE_PACKAGES).size} Packages
 ${md5file.sync(`${PATH_FILE_PACKAGES}.bz2`)} ${fs.statSync(`${PATH_FILE_PACKAGES}.bz2`).size} Packages.bz2`)
}